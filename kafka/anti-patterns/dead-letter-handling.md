...
**Why it fails:** A deserialization error or a null field in the payload will never succeed no matter how many times you retry. The partition is blocked — no subsequent messages are processed. Consumer lag grows, alerts fire, and the team scrambles.
...
**Instead:** Add a DLT consumer that logs failed messages with full context. Set up alerts on DLT consumer lag. Build replay tooling that re-publishes corrected messages to the original topic.
