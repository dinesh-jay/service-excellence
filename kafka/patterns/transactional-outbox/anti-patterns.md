...
**Why it fails:** You lose the ability to replay events during incident recovery. If a downstream consumer missed events (e.g., it was down), you have no source of truth to republish from. Deletion also makes debugging production issues harder — you can't query what was sent.
...
**Instead:** Partition by `aggregate_type` — either with separate queries per type or separate outbox tables per bounded context. This isolates throughput and failure domains.
