...
**Why it fails:** There is no enforcement. A producer adds a field, renames one, or changes a type, and the first sign of trouble is a deserialization exception in production. There is no way to validate compatibility before deployment.
...
**Instead:** Use `BACKWARD_TRANSITIVE` as the default. If you need to deploy producer changes first, use `FORWARD_TRANSITIVE`. Only use `NONE` in development environments, never in staging or production.
...
**Why it fails:** Avro treats a rename as a delete + add. Old consumers cannot find the original field and deserialization fails. Removing a required field (no default) means old consumers have no fallback value.
