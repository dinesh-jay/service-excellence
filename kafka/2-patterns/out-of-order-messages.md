...
Each event carries a monotonic sequence number per aggregate. The consumer tracks `lastProcessedSequence` per entity:
...
A scheduled job retries buffered events when gaps fill:
